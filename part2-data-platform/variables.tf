...
variable "domain_name" {
  description = "The registered domain name to create a Route 53 hosted zone for (e.g., 'my-domain.com')."
  type        = string
  default     = ""
}
...
variable "notification_sns_topic_arn" {
  description = "Optional SNS topic ARN used by Alertmanager to deliver monitoring notifications. Leave empty to disable."
  type        = string
  default     = ""
}

variable "enable_transfer_server" {
  description = "Enable provisioning of AWS Transfer Family (SFTP) endpoint fronting the shared EFS home directories."
  type        = bool
  default     = true
}

variable "transfer_hostname" {
  description = "Subdomain to associate with the Transfer Family SFTP endpoint (for example, sftp.dev.example.com)."
  type        = string
  default     = ""
}

variable "transfer_zone_id" {
  description = "Route53 hosted zone ID where the Transfer Family endpoint DNS record should be managed."
  type        = string
  default     = ""
}

variable "transfer_allowed_cidrs" {
  description = "CIDR blocks allowed to reach the Transfer Family endpoint."
  type        = list(string)
  default     = ["0.0.0.0/0"]
}

variable "transfer_users" {
  description = "Map of SFTP/SSH users provisioned on the Transfer Family endpoint. Key is the username and values define per-user attributes."
  type = map(object({
    public_keys = list(string)
    uid         = optional(number, 1000)
    gid         = optional(number, 100)
